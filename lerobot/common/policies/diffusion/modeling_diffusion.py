...
class DiffusionPolicy(nn.Module, PyTorchModelHubMixin):
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        if self.use_relative_traj:
            return self.select_action_rel(batch)
        else:
            return self.select_action_abs(batch)

    @torch.no_grad
    def select_action_abs(self, batch: dict[str, Tensor]) -> Tensor:
    ...
    @torch.no_grad
    def select_action_rel(self, batch: dict[str, Tensor]) -> Tensor:
        """Select a single action given environment observations.
