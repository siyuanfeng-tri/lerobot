        ...
        *,
        ...
        for key, value in data.items():
            ...
            if key not in self.data:
                if is_zarr:
                    # For data whose dtype is `object` (e.g., arbitrary length
                    # strings) we must specify a codec to serialize the data.
                    # TODO(charlie): Try other codecs including MsgPack, JSON,
                    # and possibly VLenUTF8.
                    # TODO(charlie): Also try just using fixed length strings,
                    # which can be serialized without any special codec.
                    object_codec = None
        ...
        if path is not None:
            if is_zarr:
                ...
                # TODO(sfeng): need to impl this. np doesnt like array of var
                # len strings?
                pass
